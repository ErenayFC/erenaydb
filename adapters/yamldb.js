...
class YamlDB {
  constructor(options) {
    ...
    try {
      ...
      throw new TypeError(
        'You must install "yaml" module for use this adapter.'
      );
    }
  ...
  set(db, data) {
    functions.fetchFiles(this.dbFolder, this.dbName);
    var content = YAML.parse(
      fs.readFileSync(`./${this.dbFolder}/${this.dbName}.yaml`, "utf8")
    );
    functions.set(db, data, content);
    fs.writeFileSync(
      `./${this.dbFolder}/${this.dbName}.yaml`,
