...
class JsonDB {
  constructor(options) {
    ...
    this.noBlankData = options["noBlankData"]
      ? typeof options["noBlankData"] === "boolean"
        ? options["noBlankData"]
        : false
      : false;
    this.readable = options["readable"]
      ? typeof options["readable"] === "boolean"
        ? true
        : false
      : false;
  ...
  set(db, data) {
    ...
    var content = JSON.parse(
      fs.readFileSync(`./${this.dbFolder}/${this.dbName}.json`, "utf8")
    );
    functions.set(db, data, content);

    if (this.readable) {
      fs.writeFileSync(
        `./${this.dbFolder}/${this.dbName}.json`,
  ...
  subtract(db, number) {
    ...
    this.set(
      db,
      this.get(db)
        ? this.get(db) - Number(number) <= 1
          ? 1
          : (isNaN(this.get(db)) ? 1 : this.get(db) - Number(number)) || 1
        : 1
    );
    return this.get(db);
  }
